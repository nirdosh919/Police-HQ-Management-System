                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#0B1F3A] font-bold text-[#C9A227]">
                            {officer.photograph ? (
                              <img
                                src={`http://127.0.0.1:5000${encodeURI(officer.photograph.startsWith("/") ? officer.photograph : "/" + officer.photograph)}?v=${Date.now()}`}
                                alt={officer.name}
                                className="absolute inset-0 h-full w-full object-cover"
                                onError={(event) => {
                                  event.currentTarget.style.display = "none";
                                  const fallback = event.currentTarget.parentElement?.querySelector("[data-photo-fallback]") as HTMLElement | null;
...
function ViewOfficerModal({
  ...
  const photoUrl = officer.photograph ? (officer.photograph.startsWith("http") ? officer.photograph : `http://127.0.0.1:5000${encodeURI(officer.photograph.startsWith("/") ? officer.photograph : "/" + officer.photograph)}`) : "";
