...
export default function Dashboard() {
  ...
  const suspended = data?.totals.suspended || 0;
  const retired = data?.totals.retired || 0;

  const maxDepartmentCount = useMemo(() => {
    return Math.max(
      ...(data?.departments.map(
        (department) => department.officers
      ) || [1]),
      1
    );
  }, [data]);
  ...
  const stats = [
    ...
    },
    {
      ...
      subtitle: `${getPercentage(active, total)}% of total`,
      icon: UserCheck,
    },
    {
      title: "Departments",
      value: formatNumber(
        data?.departmentTotal || 0
      ),
    ...
    },
    {
      ...
      value: formatNumber(retired),
      subtitle: `${getPercentage(retired, total)}% of total`,
      icon: UserRoundCheck,
    },
  ];
  ...
  return (
    <div>
      ...
      <div className="mt-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        ) : data?.recentOfficers.length ? (
          <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {data.recentOfficers.map((officer) => (
              <div
                key={officer._id}
