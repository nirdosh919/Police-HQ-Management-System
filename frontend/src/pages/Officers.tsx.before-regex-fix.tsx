                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#0B1F3A] font-bold text-[#C9A227]">
                            {officer.photograph ? (
                              <img
                                key={officer.photograph}
                                src={`http://127.0.0.1:5000${officer.photograph.startsWith("/") ? "" : "/"}${officer.photograph}`}
                                alt={officer.name}
                                className="absolute inset-0 z-10 h-full w-full object-cover"
                                onError={(event) => {
                                  console.error("OFFICER PHOTO FAILED:", event.currentTarget.src);
                                  event.currentTarget.style.display = "none";
                                  const fallback = event.currentTarget.parentElement?.querySelector("[data-officer-photo-fallback]") as HTMLElement | null;
                                  if(fallback) fallback.style.display = "flex";
                                }}
                              />
                            ) : null}
                            <span
                              data-officer-photo-fallback
