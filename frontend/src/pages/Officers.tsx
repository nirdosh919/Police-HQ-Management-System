...
function OfficerPhoto({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        console.error("OFFICER PHOTO LOAD FAILED:", error);
        if (!cancelled) {
          setFailed(true);
        }
      }
  ...
  return (
    <div
      className={`${box} flex shrink-0 items-center justify-center rounded-full bg-[#0B1F3A] font-bold text-[#C9A227]`}
    >
      {getInitials(name)}
    </div>
  );
