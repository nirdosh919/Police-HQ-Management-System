...
export default function EditOfficerModal({
  ...
  const photoPreviewUrl = photograph
    ? URL.createObjectURL(photograph)
    : officer.photograph
      ? officer.photograph.startsWith("http")
        ? officer.photograph
        : `http://localhost:5000${officer.photograph.startsWith("/") ? "" : "/"}${officer.photograph}`
      : "";
...
const removeDocument = (index: number) => {
  ...
  <img
    src={photoPreview || photoPreviewUrl}
    alt={officer.fullName}
    className="h-full w-full object-cover"
    onError={(event) => {
      console.error("PHOTO PREVIEW FAILED:", photoPreviewUrl);
